...
pub fn update_daily_models_cache() -> Result<()> {
    ...
    let tree_response = client.get(tree_url)
    ...
    if !tree_response.status().is_success() {
        anyhow::bail!("Failed to fetch tree models: status {}", tree_response.status());
    }
    ...
    if !search_response.status().is_success() {
        anyhow::bail!("Failed to fetch search models: status {}", search_response.status());
    }

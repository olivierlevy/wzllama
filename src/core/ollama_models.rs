...
impl ModelConfig {
    ...
    pub fn env_vars(&self) -> Vec<(String, String)> {
        ...
        if self.flash_attention {
            v.push(("OLLAMA_FLASH_ATTENTION".into(), "1".into()));
        }
        v
    }
...
pub fn score_model(model: &OllamaModel, usage: &str, hw: &HardwareInfo) -> f32 {
    ...
    if name.contains("cloud") || name.contains("remote") {
        return 0.0;
    }
    ...
    for kw in keywords {
        if name.contains(kw) || family.contains(kw) {
            score += 0.1;
        }
    }
...
pub fn rank_models(
    ...
    let mut scored: Vec<_> = models
        .iter()
        .filter(|m| {
            let s = extract_size(&m.name);
            s > 0 && !m.name.to_lowercase().contains("cloud")
        })
        .map(|m| {
            let s = score_model(m, usage, hw);
            (m.clone(), s)
        })
        .filter(|(_, s)| *s > 0.0)
        .collect();
...
pub fn recommend_config(
    ...
    let (num_ctx, kv, flash, temp, prompt): (u32, String, bool, f32, Option<String>) = match task {
        ...
        TaskType::MultiAgent { agent_count } => {
            ...
            (
                ...
                true,
                0.9,
                Some(i18n.t_with_vars(
                    "config.prompt.multi_agent",
                    &[("count", &agent_count.to_string())],
                )),
            )
        }
        TaskType::Rag => (
            max_ctx.clamp(4096, 16384),
            "q8_0".into(),
            true,
            0.5,
    ...
    let (num_ctx, kv, flash) = if num_ctx < 2048 {
        (2048, "q4_0".into(), true)
    } else {
        (num_ctx, kv, flash)
    };

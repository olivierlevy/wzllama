...
pub fn build_main_menu(i18n: &I18n, state: &WzllamaState, _hw: &HardwareInfo) -> Result<MenuTree> {
    ...
    if has_resume {
        if let Some(ref last_tool) = state.last_tool {
            if let Some(tool) = tools::get_tool(last_tool) {
                items.push(
                    MenuItem::leaf(&format!("▶ Reprendre {}", tool.name()))
    ...
    let root = MenuItem::branch(&i18n.t("menu.main.title")).add_submenus(items);
...
pub fn build_wizard_menu(i18n: &I18n) -> MenuTree {
    let items = vec![
        ...
        MenuItem::leaf(&i18n.t("wizard.usecase.multimodal"))
            .with_action("wizard_usecase_multimodal"),
    ...
    MenuTree::new(&i18n.t("wizard.title")).with_root(root)
}
...
pub fn build_models_menu(i18n: &I18n, state: &WzllamaState) -> MenuTree {
    ...
    for model in local_models {
        let default_marker = if Some(model.name.clone()) == last_model {
            " (default)"
        } else {
            ""
        };
        let label = format!(
            ...
            model
                .details
                .as_ref()
                .map(|d| d.parameter_size.as_deref().unwrap_or(""))
                .unwrap_or(""),
            default_marker
        );
    ...
    let root = MenuItem::branch(&i18n.t("menu.main.models")).add_submenus(items);

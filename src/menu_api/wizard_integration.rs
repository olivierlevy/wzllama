...
use crate::menu_api::{
    MenuTree, MenuItem, ActionDispatcher, ActionResult, ClosureAction,
};

/// Type alias for wizard function signatures
pub type WizardFunc = Arc<dyn Fn(&I18n, &mut WzllamaState, &HardwareInfo) -> Result<()> + Send + Sync>;
...
impl WizardMenuBuilder {
    ...
    pub fn add_item(mut self, label: String, action_id: Option<String>, func: Option<WizardFunc>) -> Self {
    ...
    pub fn add_submenu(mut self, label: String, children: Vec<(String, Option<String>, Option<WizardFunc>)>) -> Self {
        self.items.push(MenuItemData {
            label,
            action_id: None,
            func: None,
            children: children.into_iter().map(|(label, action_id, func)| MenuItemData {
                label,
                action_id,
                func,
                children: Vec::new(),
    ...
    pub fn build(self, _dispatcher: &mut ActionDispatcher) -> MenuTree {
        let menu_items: Vec<MenuItem> = self.items.iter()
            .map(|item| {
                ...
                if !item.children.is_empty() {
                    let child_items: Vec<MenuItem> = item.children.iter()
                        .map(|child| {
                            MenuItem::leaf(&child.label).with_action(child.action_id.as_deref().unwrap_or(""))
                        })
                        .collect();
...
pub fn create_main_menu_structure(i18n: &I18n, state: &WzllamaState) -> WizardMenuBuilder {
    ...
    builder = builder
        ...
        .add_item(i18n.t("menu.main.scientific"), Some("scientific".to_string()), None)
        .add_item(i18n.t("menu.main.tools"), Some("tools".to_string()), None)
        .add_item(i18n.t("menu.main.cleanup"), Some("cleanup".to_string()), None)
        .add_item(i18n.t("menu.main.config"), Some("config".to_string()), None)
        .add_item(i18n.t("menu.main.language"), Some("language".to_string()), None)
...
pub fn build_dispatcher(_i18n: &I18n) -> ActionDispatcher {
    ...
    dispatcher.register(Box::new(ClosureAction::new("scientific", "Scientific", |_ctx| {
        Ok(ActionResult::success())
    })));
    ...
    dispatcher.register(Box::new(ClosureAction::new("language", "Language", |_ctx| {
        Ok(ActionResult::success())
    })));
...
pub fn run_wizard_menu<'a>(i18n: &'a I18n, state: &'a mut WzllamaState, hw: &'a HardwareInfo) -> Result<()> {

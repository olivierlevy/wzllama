...
mod menu_api_tests {
    use crate::menu_api::{
        ActionContext, ActionDispatcher, ActionResult, ClosureAction, MenuHandler, MenuItem,
        MenuTree,
    };

    fn setup_test_tree() -> MenuTree {
        let root = MenuItem::branch("Main")
            .add_submenu(MenuItem::leaf("Install Ollama").with_action("install_ollama"))
            .add_submenu(
            ...
            .add_submenu(MenuItem::leaf("Quit").with_action("quit_app"));
    ...
    fn setup_dispatcher() -> ActionDispatcher {
        ...
        dispatcher.register(Box::new(ClosureAction::new("quit_app", "Quit", |_| {
    ...
    fn test_menu_item_is_leaf() {
        ...
        let branch = MenuItem::branch("Branch").add_submenu(MenuItem::leaf("Child"));
    ...
    fn test_action_dispatcher_register() {
        ...
        dispatcher.register(Box::new(ClosureAction::new("test_action", "Test", |_| {
            Ok(ActionResult::success())
        })));
    ...
    fn test_scientific_category_variants() {
        ...
        let all_skills: Vec<&str> = all.iter().flat_map(|c| c.skills.iter().copied()).collect();
    ...
    fn test_retour_in_position_0_pattern() {
        ...
        assert!(
            cleanup.root.submenus[0].label.contains("Retour"),
        ...
        assert!(
            wizard.root.submenus[0].label.contains("Retour"),
        ...
        assert!(
            models.root.submenus[0].label.contains("Retour"),
        ...
        assert!(
            tools.root.submenus[0].label.contains("Retour"),
        ...
        assert!(
            scientific.root.submenus[0].label.contains("Retour"),
        ...
        assert!(
            config.root.submenus[0].label.contains("Retour"),

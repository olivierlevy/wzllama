...
use crate::display;
use crate::menu_api::wizard_helpers::{UseCase, get_priority_tools_for_usecase};
use crate::menu_api::{enter_alternate_screen, exit_alternate_screen, MenuTree, MenuItem, MenuMetadata};
...
impl<'a> WizardEngine<'a> {
    ...
    fn handle_usecase_selection(&mut self, use_case: UseCase) -> Result<bool> {
        ...
        let available: Vec<_> = api_models.iter()
        ...
        if let Some(ref model) = self.state.last_model {
            all_model_choices.push(self.i18n.t_with_vars("wizard.action.launch_with_current", &[("model", model)]));
        } else {
        ...
        match sel {
            ...
            Some(s) if s <= model_names.len() => {
                ...
                display::success(&self.i18n.t_with_vars("wizard.model_selected", &[("model", selected_model)]));
    ...
    fn launch_tool_for_usecase(&mut self, use_case: &UseCase, model: &str) -> Result<()> {
        ...
        let installed_tools: Vec<String> = tools.into_iter()
            .filter(|tool_id| {
                ...
                tool.as_ref().map(|t| t.status(self.state) == ToolStatus::Installed).unwrap_or(false)
            })
            .collect();
        ...
        let tool_displays: Vec<String> = installed_tools.iter()
            .filter_map(|tool_id| {
                tools::get_tool(tool_id).map(|t| {
                    format!("🔧 {} - {}", t.name(), t.description(self.i18n))
                })
            })
            .collect();
...
pub fn build_menu_tree(i18n: &I18n) -> MenuTree {
    ...
    items.extend(use_cases.iter().map(|uc| {
        MenuItem::leaf(&uc.display_name(i18n)).with_action(&format!("wizard_usecase_{}", uc.as_str()))
    }));

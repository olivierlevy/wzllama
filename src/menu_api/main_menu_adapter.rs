...
impl<'a> MainMenuRunner<'a> {
    ...
    pub fn menu_tree(&self) -> MenuTree {
        ...
        root = root
            .add_submenu(
                MenuItem::leaf(&self.i18n.t("menu.main.wizard")).with_action("menu_wizard"),
            )
            .add_submenu(
                MenuItem::leaf(&self.i18n.t("menu.main.models")).with_action("menu_models"),
            )
            .add_submenu(
                MenuItem::leaf(&self.i18n.t("menu.main.scientific")).with_action("menu_scientific"),
            )
            .add_submenu(MenuItem::leaf(&self.i18n.t("menu.main.tools")).with_action("menu_tools"))
            .add_submenu(
                MenuItem::leaf(&self.i18n.t("menu.main.cleanup")).with_action("menu_cleanup"),
            )
            .add_submenu(
                MenuItem::leaf(&self.i18n.t("menu.main.config")).with_action("menu_config"),
            )
            .add_submenu(
                MenuItem::leaf(&self.i18n.t("menu.main.language")).with_action("menu_language"),
            );
    ...
    pub fn run_with_menu_handler(&mut self) -> Result<()> {
        ...
        use crate::wizard::{
            menu_cleanup, menu_config, menu_models, menu_scientific, menu_tools, menu_wizard,
        };
        ...
        // Register language setter actions for each available language (set_language_<code>)
        {
            ...
            for lang in languages {
                ...
                // Use action_runner to create an action that updates persisted state and reloads global i18n
                dispatcher.register(Box::new(action_runner.create_action(
                    &id,
                    &name,
                    move |_i18n, state, _hw| {
                        crate::config::state::set_language(&code, state);

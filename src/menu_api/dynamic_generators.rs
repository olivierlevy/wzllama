...
pub fn generate_models_menu(
    ...
    for model in models {
        ...
        items.push(MenuItem::leaf(&label).with_action(&format!("select_model_{}", model)));
    }
...
pub fn generate_tools_menu(i18n: &I18n, state: &WzllamaState, _hw: &HardwareInfo) -> Vec<MenuItem> {
    ...
    for tool in tools_list {
        ...
        let label = format!(
            "{} {} - {}{}",
            icon, tool.name, tool.description, agentic_tag
        );

        items.push(MenuItem::leaf(&label).with_action(&format!("tool_{}", tool.id)));
    }
...
pub fn generate_scientific_menu(
    i18n: &I18n,
    state: &WzllamaState,
    _hw: &HardwareInfo,
) -> Vec<MenuItem> {

...
impl ShellConfig {
    fn all() -> Vec<Self> {
        ...
        vec![
            Self { name: "bash".into(), path: home.join(".bashrc") },
            Self { name: "zsh".into(), path: home.join(".zshrc") },
            Self { name: "fish".into(), path: home.join(".config/fish/config.fish") },
        ]
    }
    ...
    fn uninstall(&self) -> Result<()> {
        if !self.exists() || !self.has_source() { return Ok(()); }
...
pub fn install_all_shells(i18n: &I18n) -> Result<()> {
    for shell in ShellConfig::all() {
        if shell.exists() {
            shell.install()?;
            display::success(&i18n.t_with_vars("config.shell.installed", &[("shell", &shell.name)]));
        }
    }
    Ok(())
}
...
pub fn uninstall_all_shells(i18n: &I18n) -> Result<()> {
    for shell in ShellConfig::all() {
        if shell.has_source() {
            shell.uninstall()?;
            display::success(&i18n.t_with_vars("config.shell.uninstalled", &[("shell", &shell.name)]));
        }
    }
    Ok(())
}

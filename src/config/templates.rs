...
pub struct UsageParams {
    pub r#type: String,
    #[serde(default)] pub pages_per_chunk: Option<u32>,
    #[serde(default)] pub loc_per_chunk: Option<u32>,
    #[serde(default)] pub context_tokens: Option<u32>,
    #[serde(default)] pub max_tokens_per_call: Option<u32>,
}

#[derive(Debug, Deserialize, Serialize, Clone)]
pub struct UsageSpec {
    pub i18n_key: String,
    #[serde(default)] pub description_key: Option<String>,
...
pub fn ensure() -> Result<()> {
    ...
    let exe_dir = std::env::current_exe()?.parent().unwrap_or(std::path::Path::new(".")).to_path_buf();
    let cfg_source = if exe_dir.join("config").exists() { exe_dir.join("config") } else { std::path::PathBuf::from("config") };
    ...
    for file in &["default_usages.yaml", "default_config.yaml"] {
        ...
        if !dest.exists() && src.exists() { fs::copy(&src, &dest)?; }
    }
...
pub fn default_usages() -> UsagesConfig {
    ...
    usages.insert("big_book".into(), UsageSpec {
        ...
        weights: { let mut w = HashMap::new(); w.insert("default".into(), 0.7); w },
        params: UsageParams { r#type: "book".into(), pages_per_chunk: Some(20), loc_per_chunk: None, context_tokens: Some(8192), max_tokens_per_call: None },
    });
    usages.insert("big_code".into(), UsageSpec {
        i18n_key: "usage.big_code.label".into(),
        description_key: Some("usage.big_code.description".into()),
        weights: { let mut w = HashMap::new(); w.insert("default".into(), 0.6); w },
        params: UsageParams { r#type: "code".into(), pages_per_chunk: None, loc_per_chunk: Some(500), context_tokens: Some(4096), max_tokens_per_call: None },
    });
    usages.insert("fast_agents".into(), UsageSpec {
        i18n_key: "usage.fast_agents.label".into(),
        description_key: Some("usage.fast_agents.description".into()),
        weights: { let mut w = HashMap::new(); w.insert("default".into(), 0.9); w },
        params: UsageParams { r#type: "agents".into(), pages_per_chunk: None, loc_per_chunk: None, context_tokens: Some(2048), max_tokens_per_call: Some(1024) },
    });
    usages.insert("mixed".into(), UsageSpec {
        ...
        params: UsageParams { r#type: "mixed".into(), pages_per_chunk: None, loc_per_chunk: None, context_tokens: Some(4096), max_tokens_per_call: None },
    });
    UsagesConfig { usages }
}
...
pub fn reset_all() -> Result<()> {
    ...
    for file in &["usages.yaml", "config.yaml"] {
        let p = cfg.join(file);
        if p.exists() { fs::rename(&p, p.with_extension("yaml.bak"))?; }
    }

...
fn models_wizard(i18n: &I18n, state: &mut WzllamaState, hw: &HardwareInfo) -> Result<()> {
    loop {
        ...
        let display_names: Vec<String> = use_cases.iter().map(|uc| uc.display_name(i18n)).collect();
        ...
        match sel {
            ...
            Some(s) if s <= use_cases.len() => {
                if handle_usecase_selection(i18n, state, hw, use_cases[s - 1])? {
                    // -1 car Retour est en position 0
...
pub fn handle_usecase_selection(
    i18n: &I18n,
...
pub fn launch_tool_for_usecase(
    ...
    let tool_displays: Vec<String> = installed_tools
        .iter()
        .filter_map(|tool_id| {
            tools::get_tool(tool_id).map(|t| format!("🔧 {} - {}", t.name(), t.description(i18n)))
        })
        .collect();

...
use crate::core::{
    cache,
    localmax_models::{self, LocalMaxModel},
    ollama_api, ollama_models, HardwareInfo,
};
use crate::display;
...
pub fn run(i18n: &I18n, state: &mut WzllamaState, hw: &HardwareInfo) -> Result<()> {
    ...
    let _local = ollama_api::detect_url()
    ...
    let models =
        localmax_models::fetch_models_by_search("performance", 100).unwrap_or_else(|_| Vec::new());
    ...
    'outer: loop {
        ...
        for ollama_model in &local {
            ...
            let params = if extracted > 0 {
                ...
                format!(
                    "{}b",
                    crate::core::localmax_models::extract_param_size(&local_model.hf_id)
                        .trim_end_matches('b')
                )
            };
            ...
            let display = format!(
                "✅ {} [{}] {} (installed)",
                name_colored, params, local_model.organization
            );
        ...
        orgs.sort_by(|a, b| {
            let a_popularity: u32 =
                a.1.iter()
                    .map(|m| m._count.as_ref().map_or(0, |c| c.benchmark_runs))
                    .sum();
            let b_popularity: u32 =
                b.1.iter()
                    .map(|m| m._count.as_ref().map_or(0, |c| c.benchmark_runs))
                    .sum();
        ...
        main_items.push((
            format!("─── {} ───", i18n.t("models.localmaxxing_by_org")),
            None,
        ));
    ...
    if local_names.contains(model_name.as_str()) {
        ...
        if confirm {
            if let Err(e) = ollama_api::pull_model(&model_name) {
                ...
                display::success(
                    &i18n.t_with_vars("models.downloaded_success", &[("model", &model_name)]),
                );
            }
        }
    }
...
fn show_org_models_menu(
    ...
    loop {
        ...
        for model in models {
            ...
            let display = format!(
                "{} {} [{}]{}",
                icon, display_name_colored, params, fallback_indicator
            );
    ...
    loop {
        ...
        match action_sel {
            ...
            4 => {
                ...
                if Confirm::new()
                    .with_prompt(
                        i18n.t_with_vars("models.manage_delete_confirm", &[("model", model_name)]),
                    )
                    .default(false)
                    .interact()?
                {
...
fn show_model_details(i18n: &I18n, model: &ollama_api::OllamaModel, hw: &HardwareInfo) {
    ...
    if model_gb > hw.available_disk_gb {
        println!(
            "  {} ⚠️ {} {:.0}GB needed, {:.0}GB available",
            i18n.t("models.info_disk"),
            "⚠️".yellow(),
            model_gb,
            hw.available_disk_gb
        );
    } else {
        println!(
            "  {}: {:.0}GB available",
    ...
    if let Ok(details) = ollama_api::get_model_details(&model.name) {
        ...
        if let Some(param_size) = details
            .details
            .as_ref()
        ...
        if let Some(ref license) = details.license {
            ...
            println!(
                "  {}: {}",
                i18n.t("models.info_license"),
                short_license.dimmed()
            );
        }
    ...
    match hw_compat {
        "🟢" => println!(
            "  {} 🚀 {}",
            "Hardware:".green(),
            "Excellent fit for your system".green()
        ),
        "🟡" => {
            if has_gpu && size_gb > vram_gb {
                println!(
                    "  {} ✅ {}",
                    "Hardware:".green(),
                    "Good fit (RAM fallback - slower)".yellow()
                );
            } else {
        ...
        "🟠" => {
            if has_gpu && size_gb > vram_gb {
                println!(
                    "  {} ⚠️ {}",
                    "Hardware:".yellow(),
                    "Fits in RAM only - slower performance".yellow()
                );
            } else {
                println!(
                    "  {} ⚠️ {}",
                    "Hardware:".yellow(),
                    "Fits with constraints".yellow()
                );
            }
        }
        _ => println!(

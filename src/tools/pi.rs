...
impl Tool for PiTool {
    ...
    fn description(&self, i18n: &I18n) -> String {
        i18n.t("tool.pi.description")
    }
    fn status(&self, _state: &WzllamaState) -> ToolStatus {
        if shell::is_installed_with_local_bin("pi") {
...
impl PiTool {
    ...
    pub fn uninstall(i18n: &I18n) -> Result<()> {
        if !Confirm::new()
            .with_prompt(i18n.t("tool.pi.uninstall_confirm"))
            .default(false)
            .interact()?
        {
            return Ok(());
        ...
        #[cfg(unix)]
        {
            let _ = shell::run_quiet(
                "sudo npm uninstall -g @earendil-works/pi-coding-agent 2>/dev/null",
            )
            .ok();
